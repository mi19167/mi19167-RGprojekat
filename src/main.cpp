...
Shader &drawFishFetchShader(Shader &shaderGeometry, Model &riba,
                            const vector<glm::vec3> &objectPositions, glm::mat4 &modelDS);
...
int main() {
    ...
    unsigned int cubeTexture =
        loadTexture(FileSystem::getPath("resources/textures/kamencici.jpg").c_str());
    unsigned int floorTexture =
        loadTexture(FileSystem::getPath("resources/textures/kamencici.jpg").c_str());
    unsigned int transparentTexture =
        loadTexture(FileSystem::getPath("resources/textures/grass.png").c_str());
    ...
    while (! glfwWindowShouldClose(window)) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, gBuffer);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glm::mat4 projectionDS = glm::perspective(
        ...
        // draw objects
        shaderPod.use();
        glm::mat4 projectionP = glm::perspective(
            glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        ...
        model1 = glm::scale(
            model1,
            glm::vec3(0.9f, 1.0f, 0.5f)); // it's a bit too big for our scene, so scale it down

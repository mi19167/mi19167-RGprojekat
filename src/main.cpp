...
auto main() -> int
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Fishes", nullptr, nullptr);
    if (window == nullptr)
    {
    ...
    // load textures
    // -------------
    unsigned int cubeTexture = loadTexture(FileSystem::getPath("resources/textures/kamencici.jpg").c_str());
    unsigned int floorTexture = loadTexture(FileSystem::getPath("resources/textures/kamencici.jpg").c_str());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, gBuffer);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glm::mat4 projectionDS = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        glm::mat4 viewDS = camera.GetViewMatrix();
        ...
        // --------------------------------
        shaderLightBox.use();
        shaderLightBox.setMat4("projectionDS", projectionDS);
        shaderLightBox.setMat4("viewDS", viewDS);
        for (unsigned int i = 0; i < lightPositions.size(); i++)
        {
            modelDS = glm::mat4(1.0f);
            modelDS = glm::translate(modelDS, lightPositions[i]);
            modelDS = glm::scale(modelDS, glm::vec3(0.125f));
            shaderLightBox.setMat4("modelDS", modelDS);
        ...
        // draw objects
        shaderPod.use();
        glm::mat4 projectionP = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        glm::mat4 viewP = camera.GetViewMatrix();
        shaderPod.setMat4("projectionP", projectionP);
        shaderPod.setMat4("viewP", viewP);
        // set light uniforms
        shaderPod.setVec3("viewPos", camera.Position);
        shaderPod.setVec3("lightPos", lightPos);
        shaderPod.setInt("blinn", blinn);
        // floor
        ...
        ourShader.setFloat("material.shininess", 31.0f);
        ...
        model1 = glm::translate(model1,
                                glm::vec3(0.0f, 0.5f, 2.0f)); // translate it down so it's at the center of the scene
        model1 = glm::scale(model1, glm::vec3(0.9f, 1.0f, 0.5f));    // it's a bit too big for our scene, so scale it down
